...
const pillarData = [
  {
    ...
    description:
      "First things first, I'd love to know who I'm talking to and how to best reach you.",
    questions: [
      {
        id: "c_name",
        text: "Your Full Name",
  ...
  },
  {
    ...
    description:
      "Tell me a little bit about what you do, who you serve, and what you want this website to achieve.",
    questions: [
      {
        id: "p1_q1",
      ...
      },
      {
        ...
        placeholder:
          "e.g. I want people to easily find my menu, book an appointment, call me...",
        type: "textarea",
        required: true,
      },
    ],
  },
  {
    ...
    description:
      "All of my websites come standard with fast load times, mobile responsiveness, smooth functionality, and basic SEO. To make this project fit your business, choose where you want the deepest focus.",
    ...
    questions: [
      {
        ...
        options: [
          {
            value: "visuals",
            label: "🎨 Custom Branding & Visuals",
            desc: "A sharper visual identity and a stronger premium feel.",
          },
          {
            value: "ecommerce",
            label: "🛍️ E-commerce / Selling",
            desc: "Sell products, deposits, or gift cards directly from the site.",
          },
          {
            value: "reviews",
            label: "⭐ Customer Trust & Reviews",
            desc: "Showcase social proof and turn reputation into conversions.",
          },
          {
            value: "portfolio",
            label: "📸 Project Galleries / Menus",
            desc: "Present offerings clearly with visual depth and easy browsing.",
          },
          {
            value: "portals",
            label: "🔐 Customer Portals / Login",
            desc: "Private spaces for clients to access history, files, or updates.",
          },
          {
            value: "contact",
            label: "✉️ Advanced Quote Forms",
            desc: "Capture more useful lead details before the first call.",
          },
        ],
      },
    ],
  },
  {
    ...
    description:
      "A few final details so I can understand your timeline and competitive landscape.",
    questions: [
      {
        id: "p4_q1",
  ...
  if (question.type === "textarea") {
    ...
    control = createElement("input", {
      ...
      attributes: {
        ...
        inputmode: question.inputType === "email" ? "email" : undefined,
      },
    });
    control.value = state.answers[question.id] || "";
  }
...
function buildChecklistQuestion(question) {
  ...
  question.options.forEach((option) => {
    const optionLabel = createElement("label", {
      className: "feature-list__option",
    });
    ...
    const check = createElement("span", {
      className: "feature-list__check",
      attributes: { "aria-hidden": "true" },
    });
    const checkIcon = createElement("span", {
      className: "feature-list__check-icon",
      text: "✓",
    });
...
function renderStep() {
  ...
  step.questions.forEach((question) => {
    list.appendChild(
      question.type === "top5_checklist"
        ? buildChecklistQuestion(question)
...
function handleChecklistChange(target) {
  const selectedValues = Array.from(
    elements.wizardForm.querySelectorAll(
  ...
  if (selectedValues.length > MAX_PRIORITIES) {
    ...
    showToast(
      "Top 5 Reached",
      `You can only select up to ${MAX_PRIORITIES} core priorities. Remove one selection before adding another.`,
    );
    return;
  }
...
function markInvalid(target, message) {
  ...
  target.classList.add(
    target.matches(".form-control")
      ? "form-control-error"
      : "questionnaire__item-error",
  );
  ...
  for (const question of step.questions) {
    ...
    if (question.type === "top5_checklist") {
      ...
      if (question.required && selections.length === 0) {
        return markInvalid(
          target,
          "Please select at least one core priority before moving on.",
        );
      }
      continue;
    }
    ...
    if (
      ...
      if (!target.reportValidity()) {
        ...
        return markInvalid(
          target,
          "Please complete the required field before moving on.",
        );
      }
    }
  }
...
async function submitFinalScope() {
  ...
  try {
    ...
    showToast(
      "Submission Failed",
      "Could not send your project scope. Please try again.",
    );
  } finally {
...
function showResults() {
  ...
  window.setTimeout(() => {
    const chartContainer =
      elements.complexityRadar?.closest(".chart-container");
...
function resetAllData() {
  const confirmed = window.confirm(
    "Are you sure you want to start over? All saved answers will be deleted.",
  );
  if (!confirmed) {
    return;
  }
  ...
  if (avatarRing && !avatarRing.dataset.boundHover) {
    ...
    if (avatarImg) {
      avatarRing.addEventListener("mouseenter", () =>
        avatarImg.classList.add("hero__avatar-image-scaled"),
      );
      avatarRing.addEventListener("mouseleave", () =>
        avatarImg.classList.remove("hero__avatar-image-scaled"),
      );
      avatarRing.addEventListener(
        "focus",
        () => avatarImg.classList.add("hero__avatar-image-scaled"),
        true,
      );
  ...
  document.querySelectorAll(".feature-list__option").forEach((option) => {
    ...
    if (check) {
      option.addEventListener("mouseenter", () =>
        check.classList.add("feature-list__check-highlighted"),
      );
      option.addEventListener("mouseleave", () =>
        check.classList.remove("feature-list__check-highlighted"),
      );
    }
  });
